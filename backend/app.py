...
allowed_origins = [
    "https://exportacaohevile.netlify.app",
    "http://localhost:3000"
]
...
# --- ROTA PARA CRIAR O BANCO (SÓ USAR UMA VEZ) ---
...
def register():
    ...
    if not username or not password: return jsonify({"msg": "Usuário e senha são obrigatórios"}), 400
    if User.query.filter_by(username=username).first(): return jsonify({"msg": "Usuário já existe"}), 400
    new_user = User(
        username=username,
...
# =====================================================================
# --- 4. ROTA DE GERAR FICHEIRO (Excel) ---
# =====================================================================
...
@jwt_required()
def handle_form():
    # (O seu código de geração de Excel permanece inalterado)
    if not request.json: return {"error": "Missing JSON"}, 400
    data = request.json
    workbook = openpyxl.Workbook()
    sheet = workbook.active
    sheet.title = "HBL SAMPLE"
    sheet.page_setup.orientation = 'portrait'
    try: sheet.page_setup.paper_size = sheet.page_setup.PAPERSIZE_A4
    except AttributeError: sheet.page_setup.paper_size = '9'
    ...
    thin_black_border = Border(left=Side(style='thin', color='000000'), right=Side(style='thin', color='000000'), top=Side(style='thin', color='000000'), bottom=Side(style='thin', color='000000'))
    ...
    for container in containers_list:
        try: total_package_count += int(container.get('packageCount', 0))
        except (ValueError, TypeError): pass
        try: total_gross_weight += float(container.get('grossWeight', 0))
        except (ValueError, TypeError): pass
        try: total_measurement_cbm += float(container.get('measurementCBM', 0))
        except (ValueError, TypeError): pass
    ...
    for i, container in enumerate(containers_data):
        ...
        for col_idx in range(1, 14): sheet.cell(row=current_row, column=col_idx).alignment = align_center
